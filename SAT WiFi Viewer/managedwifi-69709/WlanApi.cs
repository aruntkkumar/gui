						...
						case Wlan.WlanNotificationCodeAcm.ScanFail:
							{
                                ...
                                if (notifyData.dataSize >= expectedSize)
                                //if (notifyData.dataSize >= 0) //Refer http://stackoverflow.com/questions/42371871/issues-with-using-managed-wifi-nativewifi-api
                                {
                                    int reasonInt = Marshal.ReadInt32(notifyData.dataPtr);
                                    if (Enum.IsDefined(typeof(Wlan.WlanReasonCode), reasonInt))
                                    {
                                        //Wlan.WlanReasonCode reasonCode = (Wlan.WlanReasonCode) Marshal.ReadInt32(notifyData.dataPtr);
                                        Wlan.WlanReasonCode reasonCode = (Wlan.WlanReasonCode)reasonInt;
